...
export default function TaxSettings() {
    ...
    const [formData, setFormData] = useState({ id: '', code: '', name: '', rate: 0, accountId: '' });
    ...
    const loadData = () => {
        setIsLoading(true);
        Promise.all([
            authenticatedFetch('/api/finance/tax').then(res => res.json()),
            authenticatedFetch('/api/accounts?isPosting=true').then(res => res.json())
        ])
        .then(([taxJson, accJson]) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (json.success) {
                ...
                setFormData({ id: '', code: '', name: '', rate: 0, accountId: '' });
                setIsEditing(false);
                loadData();
            } else {
    ...
    const columns = [
        ...
        { header: 'Linked Account', accessor: (t: any) => t.account ? <span className="text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 px-2 py-1 rounded-lg">({t.account.code}) {t.account.name}</span> : <span className="text-xs text-rose-500 font-bold">Unmapped</span> },
        {
            header: 'Actions',
            accessor: (t: any) => (
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-[2rem] shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="space-y-2 md:col-span-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Linked Account (Debit/Credit Hit)</label>
                        <select 
                    ...
                    <div className="flex gap-2 items-end">
                        ...
                        {isEditing && (
                            <button onClick={() => { setIsEditing(false); setFormData({ id: '', code: '', name: '', rate: 0, accountId: '' }); }} className="p-4 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-black rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                ✕
                            </button>
                        )}
                    </div>
