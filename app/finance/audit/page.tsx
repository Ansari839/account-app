...
export default function CompanyAuditLogPage() {
    ...
    useEffect(() => {
        authenticatedFetch('/api/finance/audit')
            .then(res => res.json())
            .then(data => {
        ...
        <MainLayout>
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-white mb-1">Company Activity Log</h1>
                        <p className="text-slate-400">Track changes and actions within this company.</p>
                    </div>
                </div>
                                ...
                                {logs.map((log) => (
                                    <tr key={log.id} className="hover:bg-slate-700/30 transition-colors">
                                        ...
                                        <td className="p-4 text-xs">
                                            {log.entityId && <span className="text-slate-500">Ref: {log.entityId}</span>}
                                        </td>
                                    </tr>
                                ))}
                                {logs.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="p-8 text-center text-slate-500">No activity logs found for this company.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
